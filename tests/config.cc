...
TEST(Config_Tests, has_field) {
    using namespace Config;
    {
        ...
        Category cat = Category::ParseFromINI("../default.ini", &err);
...
TEST(Config_Tests, dump_to_ini) {
    using namespace Config;
    {
        ...
        Category cat = Category::ParseFromINI("../default.ini", &err);
        Category::DumpToINI(cat, "../Test.ini", &err_ini);
        

        EXPECT_TRUE(err.IsOk());
        EXPECT_TRUE(err_ini.IsOk());

        std::ifstream verifiable("../Test.ini");
        std::ifstream testing("../Checking_file.ini");
